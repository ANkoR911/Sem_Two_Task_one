...
public:
	...
	double PointsDistanceCube(const Point &b) const
	{
...
public:
	...
	{
		Point Rez((a.ShowX() + VecCoordX), (a.ShowY() + VecCoordY));
		return Rez;
	}
...
public:
	...
	Point ShowAPoint() const
	{
		return A;
	}

	Point ShowBPoint() const
	{
		return B;
	}
	...
	{
		return (A.ShowX() - B.ShowX())*(A.ShowX() - B.ShowX()) + (A.ShowY() - B.ShowY())*(A.ShowY() - B.ShowY());
	}
...
public:
	...
	Line(const Point &_point, const Vector &_vector) : Start(_point), Direction(_vector)
	{}

	Line(const Segment &a) : Start(a.ShowAPoint()), Direction(a.FindVector())
	{}

	void RetCoefficients(double &A, double &B, double &C) const
	{
		Point One, Two;
		One = Start;
		Two = Direction + Start;
		A = One.ShowY() - Two.ShowY();
		B = Two.ShowX() - One.ShowX();
		C = One.ShowX()*Two.ShowY() - Two.ShowX()*One.ShowY();
	}

	bool FindIntersectionPointWith(const Line &b, Point &Rez) const
	{
		if (Direction.Parallel(b.Direction))
		{
			return 0;
		}
		else
		{
			double A1, B1, C1, A2, B2, C2, X1, Y1;
			this->RetCoefficients(A1, B1, C1);
			b.RetCoefficients(A2, B2, C2);
			X1 = -((C1*B2 - C2*B1) / (A1*B2 - A2*B1));
			Y1 = -((A1*C2 - C1*A2) / (A1*B2 - B1*A2));
			Point Rez1(X1, Y1);
			Rez = Rez1;
	...
	bool FindIntersectionPointWith(const Segment &a, Point &Rez) const
	{
		Point FromB = a.ShowAPoint();
		Vector ForB = a.FindVector();
		Line b(FromB, ForB);
		Point Rez1;
		bool Help = this->FindIntersectionPointWith(b, Rez1);
		if ((Help) && (a.PointAccessory(Rez1)))
		{
			Rez = Rez1;
			return 1;
		}
	...
	friend istream& operator>>(istream& is, Line& dt);
};
...
istream& operator>>(istream& is, Line& dt)
{
	is >> dt.Start >> dt.Direction;
	return is;
}
